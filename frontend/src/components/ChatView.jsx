...
export default function ChatView({ sessionId, onNewChat, onSessionUpdated, onOpenBuilder }) {
  ...
  if (!sessionId && messages.length === 0) {
    return (
      <div className="flex-1 flex flex-col bg-[#0a0a0c]">
        <header className="px-8 py-5 border-b border-white/10 bg-black/30 backdrop-blur flex items-center gap-3">
          <BotAvatar size={36} />
          <div>
            <div className="text-[15px] font-semibold text-white">Jarvis</div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8">
          ...
          <p className="text-white/40 mt-2 text-center max-w-sm">Ask me anything — I can build apps, write code, review your projects, or just chat.</p>
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            {[
              ...
              <button key={s} onClick={() => setInput(s)} className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-white/60 hover:text-white hover:bg-white/10 text-[13px] transition-colors">
                {s}
              </button>
            ))}
          </div>
        </div>
        <ChatComposer input={input} setInput={setInput} send={send} sending={sending} attachedFile={attachedFile} onAttach={() => fileInputRef.current?.click()} onRemoveFile={() => setAttachedFile(null)} />
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#0a0a0c]">
      <header className="px-8 py-4 border-b border-white/10 bg-black/30 backdrop-blur flex items-center gap-3">
        <BotAvatar size={32} />
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
        ...
        {messages.map((m) => (
          <div key={m.id} className={`flex gap-3 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {m.role === 'assistant' && (
              <div className="w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center bg-[#0d1a2b]">
                <BotAvatar size={26} />
              </div>
            )}
        ...
        {sending && (
          <div className="flex gap-3 justify-start">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-[#0d1a2b]"><BotAvatar size={26} /></div>
        ...
        {builderAction && (
          <div className="flex justify-start">
            <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-2xl rounded-bl-sm px-4 py-3 text-[13px] text-cyan-300 max-w-[70%]">
              <div className="flex items-center gap-2 mb-2">
                <Hammer className="w-3.5 h-3.5" />
                <span className="font-medium">Builder action ready</span>
              </div>
              <p className="text-white/60 text-[12px] mb-3">{builderAction.description}</p>
              <div className="flex gap-2">
                ...
                <button onClick={dismissBuilderAction} className="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white/60 text-[12px] rounded-lg transition-colors">
                  Dismiss
                </button>
              </div>
            </div>
...
const ChatComposer = ({ input, setInput, send, sending, attachedFile, onAttach, onRemoveFile }) => (
  <form onSubmit={send} className="p-4 border-t border-white/10 bg-black/20">
    {attachedFile && (
      <div className="max-w-3xl mx-auto mb-2 flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded-lg">
        ...
        <button type="button" onClick={onRemoveFile} className="text-white/30 hover:text-white"><XIcon className="w-3.5 h-3.5" /></button>
      </div>
    )}
    <div className="max-w-3xl mx-auto flex items-end gap-2 bg-white/5 border border-white/10 rounded-2xl px-4 py-2">
      <button type="button" onClick={onAttach} title="Attach file" className="w-8 h-8 rounded-full flex items-center justify-center text-white/40 hover:text-white hover:bg-white/10 transition-colors flex-shrink-0 mb-1">
